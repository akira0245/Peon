...
namespace Peon.Gui
{
    ...
    {
        private static uint CombineColor(uint oldColor, uint newColor)
        {
            if (newColor == oldColor)
                return newColor;

            return oldColor switch
            {
                RedText    => RedText,
                PurpleText => newColor == RedText ? RedText : PurpleText,
                GreenText => newColor switch
                {
                    RedText    => RedText,
                    PurpleText => PurpleText,
                    _          => oldColor,
                },
                YellowText => newColor == LightGreenText ? YellowText : newColor,
                _          => newColor,
            };
        }

        private static uint TextToHeader(uint color)
        {
            return color switch
            {
                RedText    => RedHeader,
                PurpleText => PurpleHeader,
        ...
        private readonly struct CropInfo
        {
            ...
            public CropInfo(DateTime now, dynamic plot)
            {
                ...
                for (ushort i = 0; i < (int) plot.Beds.Length; ++i)
                {
                    ...
                    if (diff < TimeToFinish && finishTime != DateTime.UnixEpoch)
                        TimeToFinish = diff < TimeSpan.Zero ? TimeSpan.Zero : diff;
                    diff = wiltingTime - now;
                    if (diff < TimeToWilt && wiltingTime != DateTime.UnixEpoch)
                        TimeToWilt = diff < TimeSpan.Zero ? TimeSpan.Zero : diff;
                    diff = wiltingTime - nowMinus;
                    if (diff < TimeToWither && wiltingTime != DateTime.UnixEpoch)
        ...
        private static (string, string, string, uint) GetCropTimes(DateTime now, DateTime finishTime, DateTime wiltingTime)
        {
            ...
            if (finishTime == DateTime.UnixEpoch)
            {
                if (wiltingTime == DateTime.UnixEpoch)
